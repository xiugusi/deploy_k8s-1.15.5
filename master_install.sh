...
do
    if [ `grep -c "$host" /etc/hosts` -eq 0 ];then
        if [[ $host == $ipaddr ]];then
            hostnamectl set-hostname ${hostname}-master-${host##*.} && hostname ${hostname}-master-${host##*.}
            echo $host ${hostname}-master-${host##*.} >> /etc/hosts
        else
            echo $host $hostname-node-${host##*.} >> /etc/hosts
        fi
    fi
done
}
...
if [ $? -eq 0 ];then
	if [[ `grep -e 'registry-mirrors' -e 'cgroupdriver=systemd' -c /etc/docker/daemon.json` -lt 2 ]];then
...
systemctl daemon-reload && systemctl restart docker && systemctl enable docker && echo "docker已安装完毕！！" || { echo 'docker启动失败？？' ; exit 1 ; }
}
...
cp -p /root/.bash_profile /root/.bash_profile.$(date '+%Y%m%d%H%M%S')
[[ `grep -c KUBECONFIG /root/.bash_profile` -eq 0 ]] && echo "export KUBECONFIG=/root/.kube/config" >> /root/.bash_profile
...
get_token(){
cd $home_path
token_lis=`/usr/bin/kubeadm token list | awk 'NR==2{print $1}'`
sed -i 's#token.*#token='$token_lis'#' options.conf
sed -i "s#sha_value.*#sha_value=$(openssl x509 -pubkey -in /etc/kubernetes/pki/ca.crt|openssl rsa -pubin -outform der 2>/dev/null|openssl dgst -sha256 -hex|sed 's/^.* //')#" options.conf
echo "token 设置完毕"
}
...
then
  ...
  #rsync -av $home_path/ --exclude=images root@$host:/root/
  scp -r $home_path/* root@$host:/root/
  scp /etc/hosts root@$host:/etc/hosts && \
  hostname=`awk '/'$host'/{print $2}' /etc/hosts`
  ssh root@$host "hostnamectl set-hostname $hostname" && \
  ssh root@$host ./node_install.sh
fi
echo "Node节点: ${host} 安装完毕！！"
done
}
...
check_cluster(){
sleep 5
kubectl get node && kubectl cluster-info
}
...
nodes_install
check_cluster
echo "k8s_$k8s_version 集群已经安装完毕！"
}
